...
gpa = Tk()
gpa.title("CGPA APP V1") 
gpa.geometry("1245x730+0+5")
gpa.minsize(1000,500)
gpa.iconbitmap("pics/icon.ico")
...
note.add(tab3, text="MjB-DASHBOARD", image=dashI)
...
note.add(tab9, text="ADMIN DASHBOARD", image=adminI)
...
logo2 = PhotoImage(file='pics/bsuba.png')
Label(headerF, image=logo2, bg="Cyan4").grid(row=0, column=0, sticky=(W), ipady=2, padx=10)
Label(headerF, image=logo1, bg="Cyan4").grid(row=0, column=0, ipady=2,columnspan=2, padx=20)
  ...
  if(userN.get()=="") and (passW.get()==""):
    ...
    try:
      ...
      if(userN.get()==LASTNAME) and passW.get()==PIN:
        ...
        retSVal.set(str(len(scoreOUT.get_children()))+' Course Unit(s)')  
...
def sign():
  try:
    ...
    shif.msg =  "    DASHBOARD TO AVAILABLE TOOLS.            " 
    shif()  
...
cgpaYrVal = StringVar()
yrval = ['One','Two','Three','Four','Other']
...
valz1 = ['5.0','4.4','4.0','3.5','3.0','2.5','2.0','1.0']
...
def filtercgpa(*args):
  ...
  try:
    ...
    if str(cgpaYr.get())=='One':
      ...
      if(4.4 <= float((round(((sgps1+sgps2)/(scus1+scus2)),3))) < 5.1):
        degCRES.config(text='FIRST' , fg='blue')
        degWRES.config(text=str(round(((sgps1+sgps2)/(scus1+scus2)),2)))

      elif (3.6 <= float((round(((sgps1+sgps2)/(scus1+scus2)),3))) < 4.4):
        degCRES.config(text='SECOND UPPER' , fg='blue')
        degWRES.config(text=str(round(((sgps1+sgps2)/(scus1+scus2)),2)))

      elif (2.8 <= float((round(((sgps1+sgps2)/(scus1+scus2)),3))) < 3.6):
        degCRES.config(text='SECOND LOWER', fg='green')
        degWRES.config(text=str(round(((sgps1+sgps2)/(scus1+scus2)),2)))

      elif (0.0 <= float((round(((sgps1+sgps2)/(scus1+scus2)),3))) < 2.8):
        degCRES.config(text='PASS', fg='red')
        degWRES.config(text=str(round(((sgps1+sgps2)/(scus1+scus2)),2)))
      ...
      if(4.4 <= float((round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),3))) < 5.1):
        degCRES.config(text='FIRST' , fg='blue')
        degWRES.config(text=str(round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),2)))

      elif (3.6 <= float((round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),3))) < 4.4):
        degCRES.config(text='SECOND UPPER' , fg='blue')
        degWRES.config(text=str(round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),2)))

      elif (2.8 <= float((round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),3))) < 3.6):
        degCRES.config(text='SECOND LOWER', fg='green')
        degWRES.config(text=str(round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),2)))

      elif (0.0 <= float((round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),3))) < 2.8):
        degCRES.config(text='PASS', fg='red')
        degWRES.config(text=str(round(((sgps1+sgps2+sgps3)/(scus1+scus2+scus3)),2)))
      ...
      if(4.4 <= float((round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),3))) < 5.1):
        degCRES.config(text='FIRST' , fg='blue')
        degWRES.config(text=str(round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),2)))

      elif (3.6 <= float((round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),3))) < 4.4):
        degCRES.config(text='SECOND UPPER' , fg='blue')
        degWRES.config(text=str(round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),2)))

      elif (2.8 <= float((round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),3))) < 3.6):
        degCRES.config(text='SECOND LOWER', fg='green')
        degWRES.config(text=str(round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),2)))

      elif (0.0 <= float((round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),3))) < 2.8):
        degCRES.config(text='PASS', fg='red')
        degWRES.config(text=str(round(((sgps+sgps1+sgps2+sgps3)/(scus+scus1+scus2+scus3)),2)))

    #------------------------Total cgpa-----------------------
    elif(str(cgpaYr.get())=='Other'):
      print(str(cgpaYr.get()))
      pdtsql = ("SELECT sum(PRODUCT) FROM gpadata WHERE NAME=?")
      pdtval = (str(userN.get()),)
      cursor.execute(pdtsql, pdtval)
      result = cursor.fetchall()
      for PRODUCT in result:
        pass
      Pdt1.delete(0, END)
      PdtVal1.set(value=PRODUCT)
      sgps1 = float(Pdt1.get())
      print(float(Pdt1.get()))

      sumsql = ("SELECT sum(CREDITUNITS) FROM gpadata WHERE NAME=?")
      sumval = (str(userN.get()),)
      cursor.execute(sumsql, sumval)
      result = cursor.fetchall()
      for SUM in result:
        pass
      sUm1.delete(0, END)
      SumVal1.set(value=SUM)
      scus1 = float(sUm1.get())
      print(float(sUm1.get()))

      cgpaRES.config(text=str(round(((sgps1)/(scus1)),3)))

      if(4.4 <= float((round(((sgps1)/(scus1)),3))) < 5.1):
        degCRES.config(text='FIRST' , fg='blue')
        degWRES.config(text=str(round(((sgps1)/(scus1)),2)))

      elif (3.6 <= float((round(((sgps1)/(scus1)),3))) < 4.4):
        degCRES.config(text='SECOND UPPER' , fg='blue')
        degWRES.config(text=str(round(((sgps1)/(scus1)),2)))

      elif (2.8 <= float((round(((sgps1)/(scus1)),3))) < 3.6):
        degCRES.config(text='SECOND LOWER', fg='green')
        degWRES.config(text=str(round(((sgps1)/(scus1)),2)))

      elif (0.0 <= float((round(((sgps1)/(scus1)),3))) < 2.8):
        degCRES.config(text='PASS', fg='red')
        degWRES.config(text=str(round(((sgps1)/(scus1)),2)))
...
Fsc1 = LabelFrame(Fsc0, text="DASHBOARD",fg="white", font=('arial', 10, 'bold',"italic"), bg="gray")
...
scoreOUT.bind("<Double-Button-1>", edt)
scoreOUT.bind("<Button-1>", single)
...
retSVal = IntVar()
retSVal.set("0 Course Unit(s)")

Label(dashF1, text="RETURNED UNITS", font=('arial', 12, 'bold'), fg="white", bg='black', width=20).pack()
...
def entr1(*args):
  printScB.config(relief=SOLID, bg="white", fg="green", text=" Print Preview » ", font=('arial', 10, 'bold'))

def entr0(*args):
  printScB.config(relief=FLAT, bg="green", fg="white", text=" Print Preview ", font=('arial', 10, 'bold'))

Label(dashF1, text=" ", font=('arial', 1, 'bold'), height=1).pack(fill=X, pady=5)

printScB = Button(dashF2, text=' Print Preview ', image=prnt, compound=LEFT , cursor='hand2', font=('arial', 10, 'bold'), bg="green", fg="white", relief="flat")
printScB.grid(ipadx=5,pady=2, sticky=W+E)
...
Button(dashF2, text=' View GPA & CGPA ', command=gotoRes, cursor='hand2', font=('arial', 8, 'bold'), width=20, bg="black", fg="white", relief="groove").grid(pady=2, sticky=W+E)
...
display1.bind("<Control-s>", saveRe)
display1.bind("<Control-p>", priNt)
...
testsinfo = """
This is called the USER QUICK DASHBOARD MENU, here you can access multiple TOOLS by just selecting its BUTTON.
"""
...
>>>For QUICK EDIT, please DOUBLE TAP on the selected items, then edit and update.
...
>>> For QUICK 'SAVE' use short cut Ctrl+S.
>>> For QUICK 'PRINT' use short cut Ctrl+P.
